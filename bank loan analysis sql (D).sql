...
select issue_d from finance_1 where issue_d = '';
#-- Select the issue_d values from finance_1 where issue_d is an empty string

alter table  finance_1 add  column temp_col date null ; 
 #-- Add a new column named temp_col of type DATE allowing null values to finance_1 table

SET SQL_SAFE_UPDATES = 0;

update finance_1 set `issue_d` = null where `issue_d`= '';
 #-- Update the issue_d column with NULL values where issue_d is an empty string


update finance_1
set `temp_col` = str_to_date(concat("01-", `issue_d` ), '%d-%b-%y')
where `issue_d`  is not null;                                            
#-- Update the temp_col column by converting issue_d values to dates with a format of 'dd-Mon-yy'

select 	`temp_col` from finance_1;
#-- Retrieve the temp_col values from finance_1

select 	 year(`temp_col`) from finance_1;
  #-- Extract the year from the temp_col column in finance_1

select 	 year(`issue_d`) from finance_1; 
#-- Attempt to extract the year from the non-existing issue_d column (Note: This will likely result in an error)

alter table finance_1 drop column `issue_d`;  
#-- Remove the issue_d column from finance_1 table

alter table finance_1 change column `temp_col`  `issue_d` date;
#-- Rename the temp_col column to issue_d and change its data type to DATE

describe finance_1;  
#-- Display the structure of the finance_1 table
